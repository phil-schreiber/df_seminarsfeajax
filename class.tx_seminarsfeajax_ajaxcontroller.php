...
class tx_seminarsfeajax_ajaxcontroller extends tslib_pibase{
	...
	$sWhere = "WHERE typetable.event_type IN (".$cleanTypes.") AND seminars.deleted=0 AND seminars.begin_date >= ".$time;
	if ( isset($_POST['sSearch']) && $_POST['sSearch'] != "" )
	{
		$sWhere .= " AND (";
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			$searchvalue=$GLOBALS['TYPO3_DB']->fullQuoteStr( $_POST['sSearch'],'seminars' );
			$sWhere .= "".$aColumnsFilter[$i]." LIKE '%".substr($searchvalue,1,-1)."%' OR ";
		}
	...
	for ( $i=0 ; $i<count($aColumns) ; $i++ )
	{
		if ( isset($_POST['bSearchable_'.$i]) && $_POST['bSearchable_'.$i] == "true" && $_POST['sSearch_'.$i] != '' )
		{
			...
			$searchvalue=$GLOBALS['TYPO3_DB']->fullQuoteStr( $_POST['sSearch'],'seminars' );
			$sWhere .= "".$aColumnsFilter[$i]." LIKE '%".substr($searchvalue,1,-1)."%' ";
		}
	}
	...
	while ( $aRow = $GLOBALS['TYPO3_DB']->sql_fetch_assoc( $rResult ) )
	{
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			if ( $aColumns[$i] == "title" )
			{
				...
				$row[] = '<a href="unternehmen/schulungen/detailansicht/?tx_seminars_pi1%5BshowUid%5D='.$aRow[ 'sid' ].'">'.$aRow[ $aColumns[$i] ].'</a>'.$special;
